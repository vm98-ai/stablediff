...
class SelfAttention(nn.Module):
    ...
    def forward(self, x:torch.Tensor, casual_mask = False):
        ...
        # (batch_size, sequence_length, Dim) -> (batch_size, sequence_length, Dim*3) -> 3 tensors of shape (Batch_size, sequenc_length, dim)
        q, k, v = self.in_proj(x).chunk(3, dim=-1)

        # (batch_size, seq_len, DIm) -> (batch_size, seq_len, H, Dim/H) -> (batch_size, H, seq-len, Dim/H)
